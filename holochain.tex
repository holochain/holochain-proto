...
\usepackage{xurl} % doesn't work: https://i.imgur.com/FwBWCLg.png
% "hyperref loads the url package internally. Use \PassOptionsToPackage
% {hyphens}{url}\usepackage{hyperref} to pass the option to the url
% package when it is loaded by hyperref. This avoids any package option
% clashes." Source: 
% <https://tex.stackexchange.com/questions/3033/forcing-linebreaks-in-url/3034#comment44478_3034>.
% Note also this: "If the \PassOptionsToPackage{hyphens}{url} approach
% does not work, maybe it's "because you're trying to load the url
% package with a specific option, but it's being loaded by one of your
% packages before that with a different set of options. Try loading the
% url package earlier than the package that requires it. If it's loaded
% by the document class, try using \RequirePackage[hyphens]{url} before
% the document class." Source: 
...
% Not sure how to fix URLs breaking over o
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Eric Harris-Braun, Nicolas Luck, Arthur Brock},
            pdftitle={Holochain: scalable agent-centric distributed 
              computing},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            unicode=true,
            pdfkeywords={cryptography, distributed computing platform,
              agent-centric,
              scalable, distributed hash table, multi-agent systems},
            %linkcolor=magenta,
...
% Doesn't work.
% Unless someone can think of another solution, avoid manually breaking over pages.
...
Unfortunately the data-centric legacy inherited by Ethereum from the blockchain model, is immediately observable in its high compute cost\footnote{See \href{https://github.com/holochain/benchmarks}{our benchmarks against Ethereum} and difficulty in scaling.\footnote{see e.g. {this Coindesk article}(https://www.coindesk.com/information/will-ethereum-scale}, \href(here}{https://github.com/ethereum/eth2.0-specs/blob/master/specs/core/0_beacon-chain.md} for the latest spec, \href{https://www.coinlearningclub.com/zcash-technology-to-help-solve-ethereums-scaling-problem}{here} for an intermediate SNARK-based scaling proposal noting scaling issues, \href{https://twitter.com/search?q=ethereum%20scaling%20problem&src=typd}{here} for Twitter search results and \href{https://duckduckgo.com/?q=Ethereum%27s+difficulty+in+scaling&t=ffab&ia=web}{here} for DuckDuckGo results.}
...
The heart of the matter has to do with the trust any single agent has is in the system.  In \cite{yellowpaper} \hyperref{https://ethereum.github.io/yellowpaper/paper.pdf#subsection.1.1}{Section 1.1 (Driving Factors)} we read:
\begin{quote}
...
Systems where the restrictions are based on the nodes' identity, whether that be as declared by type or  authority, or collected from the history of the nodes' behaviors, are known as \term{permissioned} \cite{CaaS}.
...
The source of this paper is maintained at \url{https://github.com/holochain/holochain-proto/blob/develop/holochain.tex}.
It is available to view online at \url{https://raw.githubusercontent.com/holochain/holochain-proto/whitepaper/holochain.pdf}. 
...
\bibliographystyle{alpha} \todo{should change to natbib}
