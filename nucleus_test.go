...
func TestAppMessages(t *testing.T) {
	...
	Convey("it should fail on incorrect body types", t, func() {
		_, err := h.Send(AppProtocol, h.node.HashAddr, APP_MESSAGE, GetReq{})
		So(err.Error(), ShouldEqual, "expected AppMsg got holochain.GetReq")
	})

	Convey("it should fail on unknown zomes", t, func() {
		_, err := h.Send(AppProtocol, h.node.HashAddr, APP_MESSAGE, AppMsg{ZomeType: "foo"})
		So(err.Error(), ShouldEqual, "unknown zome: foo")
	})

	Convey("it should send and receive app messages", t, func() {
		r, err := h.Send(AppProtocol, h.node.HashAddr, APP_MESSAGE, AppMsg{ZomeType: "jsSampleZome", Body: `{"ping":"foobar"}`})
		So(err, ShouldBeNil)
